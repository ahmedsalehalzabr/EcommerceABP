...
** Check the `ConnectionStrings` in `appsettings.json` files under the `EcommerceApp.HttpApi.Host` and `EcommerceApp.DbMigrator` projects and change it if you need.
**
...
* Run `EcommerceApp.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `EcommerceApp.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `EcommerceApp.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
